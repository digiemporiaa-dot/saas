...
describe('website settings', () => {
  ...
  it('serves an active form by id, and nothing once it is switched off', async () => {
    const { getPublicFormById } = await import('@/lib/services/forms');
    const form = await prisma.form.create({
      data: {
        name: `Embedded form ${suffix}`,
        slug: `embedded-form-${suffix}`,
        isActive: true,
        fields: {
          create: [
    ...
    // Deactivating it must give the caller nothing rather than a dead form.
